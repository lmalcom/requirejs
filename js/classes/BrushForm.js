	...
	BrushForm = Form.extend({ 
		...
		setClass: function(ev, className){
			...
			controller.getClass(name, function(klass){
				...
				console.log('inside of the setClass function...klass is', typeof klass, klass); 
		...
		send: function(){ 
			...
			var className = this.$el.find('select')[0].value; 

			//reduce to one word 
			this.setClass(null, className); 

			//require that class and set into the window 
			return this; 
		},
	});  
	return BrushForm; 
